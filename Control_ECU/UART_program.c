...
void UART_clearbuffer(void){
	while (Get_Bit(UCSRA, UCSRA_RXC) == 1) { // Check if the receive buffer has data
	        volatile u8 dummy = UDR; // Read and discard the data
	    }
}
